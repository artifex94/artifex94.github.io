  ...
  return (
    <motion.div
      ...
      className={`flex flex-col border bg-surface transition-colors ${
        isOpen ? 'border-accent' : 'border-line hover:border-accent/50'
      }`}
    >
      <button
        type="button"
        ...
        className="group flex flex-col gap-3 p-5 text-left focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
      >
        <Icon
          className={`h-7 w-7 transition-colors ${isOpen ? 'text-accent' : 'text-accent/60 group-hover:text-accent'}`}
          aria-hidden="true"
        />
        <h3 className={`font-bold transition-colors ${isOpen ? 'text-accent' : 'text-white group-hover:text-accent'}`}>
          {segment.title}
        </h3>
        <p className="text-sm leading-relaxed text-secondary">{segment.hook}</p>
      </button>
